...
// ───────────── Forge 渲染命令已下线 ─────────────
// 媒体/出片工坊(pptx/video/tts/capture/fx)随产品改版整体移除;forge.rs 仅保留通用工具
// (run_with_timeout / find_chromium / path_to_file_url / forge_preflight),供 fable 文件中心
// 首帧抽取与 server preflight 复用。原 forge_build_pptx / forge_deck_to_pptx / forge_spec_to_pptx /
// forge_deck_to_video / forge_deck_fx_video / forge_tts / forge_screenshot 命令一并删除。
